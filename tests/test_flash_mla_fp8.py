...
def test_flash_mla(b, s_q, mean_sk, h_q, h_kv, d, dv, causal, varlen, torch_dtype):
    ...
    def prepare_fp8_input():
        ...
        if use_fp8:
            ...
            _, descale_q = scaled_quantize(q_float32.reshape((-1, d)), (1, 64), quant_dtype=fp8_dtype)
            # descale_q.fill_(1)
            q_fp8, _ = scaled_quantize(q_float32.reshape((-1, d)), (1, 64), quant_dtype=fp8_dtype, scale=descale_q)
    ...
    def fake_fp8():
        q_fp8_, _ = scaled_quantize(q_float32.reshape((-1, d)), (1, 64), quant_dtype=fp8_dtype, scale=descale_q.reshape((-1, d//64)))
        q_fake_float32 = scaled_dequantize(q_fp8_, descale_q.reshape((-1, d//64)), (1, 64), torch.float32).reshape(q_float32.shape)

        blocked_k_fp8_, _ = scaled_quantize(blocked_k_float32.reshape((-1, d)), (-1, 64), quant_dtype=fp8_dtype, scale=descale_k.unsqueeze(0))
        blocked_k_fake_float32 = scaled_dequantize(blocked_k_fp8_, descale_k.unsqueeze(0), (-1, 64), blocked_k_float32.dtype).reshape(blocked_k_float32.shape)
        return flash_mla_with_kvcache(
            q_fake_float32.to(torch.bfloat16),
            blocked_k_fake_float32.to(torch.bfloat16),
            block_table,
            cache_seqlens,
            dv,
            tile_scheduler_metadata,
    ...
    bytes = (total_seqlens * h_kv * d + b * s_q * h_q * d + b * s_q * h_q * dv) * (
        torch.finfo(q_fp8.dtype).bits // 8
    )
    print(
